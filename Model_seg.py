...
def feature_extractor_csi():
    ...
    out = tf.keras.layers.Conv1D(filters=int(8*scale), kernel_size=2, strides=1, padding='same', use_bias=False)(inp)
    out = tf.keras.layers.BatchNormalization()(out)
    out = tf.keras.layers.LeakyReLU(alpha=0.1)(out)
    out = tf.keras.layers.Conv1D(filters=int(16*scale), kernel_size=2, strides=1, padding='same', use_bias=False)(out)
    out = tf.keras.layers.BatchNormalization()(out)
    out = tf.keras.layers.LeakyReLU(alpha=0.1)(out)
...
def feature_extractor_csi_comb():
    ...
    #out = tf.keras.layers.Conv1D(filters=int(128*scale), kernel_size=2, strides=1, padding='same', use_bias=False)(inp)
    #out = tf.keras.layers.BatchNormalization()(out)
    #out = tf.keras.layers.LeakyReLU(alpha=0.1)(out)
    ...
    #out = tf.keras.layers.LeakyReLU(alpha=0.1)(out)
    #out = tf.keras.layers.Dropout(.25)(out)
    out = tf.keras.layers.Dense(int(128*scale),activation = 'tanh')(inp)
    return tf.keras.Model(inputs=inp, outputs=out)
...
def feature_extractor_pilot():
    ...
    out = tf.keras.layers.Conv1DTranspose(filters=int(32*scale), kernel_size=2, strides=2, padding='same', use_bias=False)(out)
    out = tf.keras.layers.BatchNormalization()(out)
    out = tf.keras.layers.LeakyReLU(alpha=0.1)(out)
    out = tf.keras.layers.Conv1DTranspose(filters=int(16*scale), kernel_size=2, strides=2, padding='same', use_bias=False)(out)
    out = tf.keras.layers.BatchNormalization()(out)
    out = tf.keras.layers.LeakyReLU(alpha=0.1)(out)
    out = tf.keras.layers.Conv1DTranspose(filters=int(8*scale), kernel_size=2, strides=2, padding='same', use_bias=False)(out)
    out = tf.keras.layers.BatchNormalization()(out)
    out = tf.keras.layers.LeakyReLU(alpha=0.1)(out)
    out = tf.keras.layers.Dropout(.25)(out)
    out = tf.keras.layers.Dense(int(128*scale),activation = 'tanh')(out)
...
def feature_extractor_pilot_comb():
    inp = tf.keras.Input(shape=(48, 128*scale))
    #out = tf.keras.layers.Conv1D(filters=int(32*scale), kernel_size=2, strides=1, padding='same', use_bias=False)(inp)
    #out = tf.keras.layers.BatchNormalization()(out)
    #out = tf.keras.layers.LeakyReLU(alpha=0.1)(out)
...
def PHY_Reconstruction_AE():
    ...
    cross_attention = tf.keras.layers.MultiHeadAttention(num_heads=4, key_dim=1)(CSI_diff,pilot_diff)
    ...
    #
    EQ_phy =  phy_branch - cross_attention -  CSI_diff - pilot_diff #mixed_feature
