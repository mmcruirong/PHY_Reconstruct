...
def feature_extractor_pilot():
    ...
    out = tf.keras.layers.LeakyReLU(alpha=0.1)(out)
    out = tf.keras.layers.Flatten()(out)
    out = tf.keras.layers.Dense(int(144*scale))(out)
    out = tf.keras.layers.Reshape([12,int(12*scale)])(out)
...
def CNN():
    inp = tf.keras.Input(shape=(48,int(128*scale)))#, activation='leaky_relu'
