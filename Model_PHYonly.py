...
def feature_extractor_csi():
    ...
    out = tf.keras.layers.Dense(int(32*scale))(out)
...
def feature_extractor_pilot():
    ...
    out = tf.keras.layers.BatchNormalization()(out)
    out = tf.keras.layers.LeakyReLU(alpha=0.1)(out)
    out = tf.keras.layers.Conv1D(filters=int(192*scale), kernel_size=3, strides=2, padding='same', use_bias=False)(out)
    out = tf.keras.layers.BatchNormalization()(out)
    out = tf.keras.layers.LeakyReLU(alpha=0.1)(out)
    out = tf.keras.layers.Flatten()(out)
...
def CNN():
    inp = tf.keras.Input(shape=(48,int(192*scale)))#, activation='leaky_relu'
    out = tf.keras.layers.Conv1D(filters=int(64*scale), kernel_size=3, strides=1, padding='same', use_bias=False)(inp)
    out = tf.keras.layers.BatchNormalization()(out)
    out = tf.keras.layers.ReLU()(out)
