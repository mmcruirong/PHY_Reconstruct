...
def NN_training(generator, discriminator, data_path, data_path1, logdir):
    ...
    for epoch in range(EPOCHS):
        ...
        for csi, pilot,phy_payload,groundtruth, label, label1,csi1, pilot1,snr in test_data:
            ...
            if epoch == 29:              
                #print("Save mat")
                scipy.io.savemat('MAT_OUT_64QAM/data%d.mat'%count, {'data': classifcation_np})
                scipy.io.savemat('MAT_OUT_64QAM/label%d.mat'%count, {'label': label_np})
                #print('BER = ', bit_error)
            ...
            if epoch == 0:
                ...
                #print("Save mat")
                scipy.io.savemat('MAT_OUT_64QAM_Origin/data%d.mat'%count, {'data_origin': label1_np})
                scipy.io.savemat('MAT_OUT_64QAM_Origin/label%d.mat'%count, {'label_origin': label_np})
                scipy.io.savemat('MAT_OUT_64QAM_Origin/sinr%d.mat'%count, {'sinr': sinr})
